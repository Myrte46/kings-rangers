...
public class Longbow extends BowItem {
    ...
    public Longbow(Item.Settings settings) { super(settings); }
            ...
            if (!itemStack.isEmpty() || bl) {
                ...
                if (!((double) f < 0.1)) {
                    ...
                    if (!world.isClient) {
                        ...
                        persistentProjectileEntity.setDamage(persistentProjectileEntity.getDamage() * (double) 2);
                        if (f == 1.0F) {
    ...
    public boolean hasOrigin(PlayerEntity user) {

        //  Get the origin component
        OriginComponent component = ModComponents.ORIGIN.get(user);

        //  Check if the player has the `ranger:kings-rangers` origin in any origin layer
        return component.getOrigins().values().stream().anyMatch(origin -> origin.getIdentifier().equals(new Identifier("origins:human")));
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        boolean hasOrigin = hasOrigin(user);

        if (!user.getAbilities().creativeMode && (bl || hasOrigin)) {
